...
const NavBar = ({currentUser, logout, landing})=>{
	...
	//<img src={Logo} className="theAlephLogo" alt="The-aleph Home"/> 
	 const userLinks= (	<ul className="nav navbar-nav navbar-right">
								...
								<li>
									<Link to={'/userpage'} className="nav-text">Profile</Link>
								</li>
    							
								<li>
									<a  onClick={e=>Logout(e)}className='ml-5 nav-text'>Log out</a>
								</li>
							</ul>
						)
	
	const authLinks= (<ul className="nav navbar-nav navbar-right" >
							<li>
								<Link  to="/signin" className='nav-text'>Log in</Link>
							</li>
							<li>
								<Link  to="/signup" className='ml-5 nav-text'>Sign up</Link>
							</li>
						</ul>
					)
		...
		return(
			<Fragment>
			{
				location.pathname==='/signup/select-password' || location.pathname==='/signup/select-gender' ||
				location.pathname==='/confirm-by-email' || location.pathname==='/confirm-by-number' ||
