...
const CreateAccount=({}) =>{
    ...
    const [progress, setProgress]= useState({registerInfo: true, selectGender: false, selectPassword: false, verifyAccount: false})
...
const setUserInfo=(firstName, lastName, countryCode, email, dob)=>{
    setUserData({...userData, firstName, lastName, countryCode,fcmToken, email, dob})
    setProgress({registerInfo: false, selectGender: true, selectPassword: false, verifyAccount: false})
}

const setUserGender=(genderId, genderPronoun, customGender)=>{
    setUserData({...userData, genderId, genderPronoun, customGender})
    setProgress({registerInfo: false, selectGender: false, selectPassword: true, verifyAccount: false})
}
...
//HERE WE WILL HANDLE REGISTER LOGIC{ ... }
    ...
    return (
        <Fragment>
            ...
            {progress.selectPassword && <SelectPassword getUserPassword={getUserPassword}/>}
