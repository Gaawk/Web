...
const OtpToMobile=({ userData, verifyByNumber, verifyByEmail, sendOtpToMobile, sendOtpToEmail, setAlert, history, location, }) =>{
	...
	const handleConfirmByEmail = () =>{
		...
		.then(res=>{
			...
			history.push({
				...
				state : {otp: res.data.otp, email: userData.email,  originalEmail: true}
			})
		})
		.catch(err=>{
			setActionLoading(false)
	...
	const handleCompareOtp = e =>{
		...
		if(isEmptyField(otp) || otpToCompare!==otp || expired===true || !isNumber(otp)){//
			if(isEmptyField(otp)){
				setError({
				...error,
...
<p className='form-header-text light-text'> We have sent you an SMS message with a code to {code}{' '}{number}
<span className='d-block form-header-text light-text'> Please enter the 6 digit code from the SMS below </span></p>
</header>
			...
			<div className="formInput">
					...
					<div className="signup-input-row">
								...
								className={error.invalidOtp || error.otpRequired ||error.otpsDontMatch || error.expired ?
									"otp-input signup-form-input-bar-error" : "otp-input signup-form-input-bar"}
								type="text"
								name="code"
								value={otp}
					...
					<div className="form-button-container">
						<Link to={ {pathname: '/confirm-by-number', state: {from: location.pathname}}}
						className= {showOtpOptions ? 'btn btn-lg btn-block form-button-secondary show-otp-options' 
						: 'btn btn-lg btn-block form-button-secondary hide-otp-options'}>
							Add another number
						</Link>
					...
					<div className="form-button-container">
						...
						className= {showOtpOptions ? 'btn btn-lg btn-block form-button-secondary last-form-button show-otp-options' 
...
export default connect(null,{verifyByNumber,sendOtpToMobile, sendOtpToEmail, setAlert})(OtpToMobile);
